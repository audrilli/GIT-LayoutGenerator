--DrawRegularPolygon_Fast.applescript
...
on mySnippet()
	tell application "Adobe InDesign 2024"
		...
		repeat with myPointCounter from 1 to myNumberOfPoints
			...
			copy {myX, myY} to end of myEntirePath
		end repeat
		set entire path of path 1 of myPolygon to myEntirePath
		--</fragment>
	end tell
end mySnippet
on myTeardown()
...
--<fragment>
--This function uses a JavaScript to get the cosine of an angle
--(AppleScript lacks trigonometric functions).
on myCosine(myAngle)
	tell application "Adobe InDesign 2024"

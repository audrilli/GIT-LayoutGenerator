...
Function main()
	...
	If myInCopy.documents.Count <> 0 Then
		...
		Select Case TypeName(myInCopy.Selection.Item(1))
			Case "Text", "TextColumn"
				If myInCopy.Selection.Item(1).Paragraphs.Count > 1 Then
...
Function myDisplayDialog(myInCopy)
	...
	If myResult = True Then
		...
		myDialog.Destroy
		mySortParagraphs myInCopy, mySortMethod, myIgnoreSpaces, myReverseSort
	Else
		myDialog.Destroy
	End If
End Function
Function mySortParagraphs(myInCopy, mySortMethod, myIgnoreSpaces, myReverseSort)
	Set myRegExp = New RegExp
	myRegExp.Pattern = "\s"
	myRegExp.IgnoreCase = True
    Set mySelection = myInCopy.selection.Item(1)
    ...
    Select Case mySortMethod
    	Case 0
    		...
    		myString = myInCopy.DoScript(myJavaScript, idScriptLanguage.idJavaScript, myArray)

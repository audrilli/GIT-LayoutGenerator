			...
			switch(app.selection[0].constructor.name){
				...
				case "TextColumn":
					if(app.selection[0].paragraphs.length > 1){
						...
						alert("Please select at least two paragraphs of text and try again.");
					}
					break;
				case "Cell":
				case "Row":
...
function mySortParagraphs(mySortMethod, myIgnoreSpaces, myReverseSort){
	...
	app.documents.item(0).windows.item(0).viewTab = ViewTabs.layoutView;
	...
	var myLastStoryIndex = myParagraphs.item(0).parentStory.insertionPoints.item(-1).index;
	var myLastSelectionIndex = myParagraphs.item(-1).insertionPoints.item(-1).index;
	if((myLastStoryIndex == myLastSelectionIndex)&&(myParagraphs.item(-1).contents != "")&&(myParagraphs.item(-1).characters.item(-1).contents != "\r")){

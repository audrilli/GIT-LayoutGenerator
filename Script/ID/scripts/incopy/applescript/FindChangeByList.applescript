...
--
--All InCopy search metacharacters are allowed in the "find what" and "change to" properties.
--
...
tell application "Adobe InCopy 2024"
	...
	if (count documents) is not 0 then
		if (count stories of active document) is not 0 then
			...
			if (count mySelection) is not 0 then
				if class of item 1 of mySelection is in myTextObjects then
					if class of item 1 of mySelection is text frame then
						...
						set myObject to object reference of item 1 of mySelection
						set myCheckSelection to true
					end if
					set {myObject, myCheckSelection} to my myDisplayDialog(myObject, myCheckSelection)
				else
			...
			if myObject is not equal to nothing then
				my myFindChangeByList(myObject, myCheckSelection)
			end if
		else
...
on myDisplayDialog(myObject, myCheckSelection)
	tell application "Adobe InCopy 2024"
		...
		tell myDialog
			tell (make dialog column)
				...
				tell myRangeButtons
					make radiobutton control with properties {static label:"Selected story", checked state:true}
					make radiobutton control with properties {static label:"Entire document"}
					if myCheckSelection is true then
		...
		if myResult is true then
			...
			if myRange is 0 then
				...
				set myObject to active document
			else if myRange is 2 then
				set myCheckSelection to true
			end if
		else
			set myObject to nothing
			destroy myDialog
		end if
		return {myObject, myCheckSelection}
	end tell
end myDisplayDialog
on myFindChangeByList(myObject, myCheckSelection)
	...
	tell application "Adobe InCopy 2024"
		...
		if myEOF > 0 then
			...
			if myCheckSelection is true then
				set myStart to index of character 1 of myObject
				set myEnd to index of character -1 of myObject
				set myStory to parent story of myObject
				set myStoryEnd to index of character -1 of myStory
				set myEnd to (myStoryEnd - myEnd) + 1
			end if
			repeat with myCounter from 1 to (count myText)
				...
				if myCheckSelection is true then
					tell myStory
						set myObject to object reference of text from character myStart to character -myEnd
					end tell
					select myObject
				end if
			end repeat
...
on myFindText(myObject, myFindPreferences, myChangePreferences, myFindChangeOptions)
	tell application "Adobe InCopy 2024"
		...
		set myScript to "tell application \"Adobe InCopy 2024\"" & return & "set properties of find text preferences to " & myFindPreferences & return
...
on myFindGrep(myObject, myFindPreferences, myChangePreferences, myFindChangeOptions)
	tell application "Adobe InCopy 2024"
		...
		set myScript to "tell application \"Adobe InCopy 2024\"" & return & "set properties of find grep preferences to " & myFindPreferences & return
...
on myFindGlyph(myObject, myFindPreferences, myChangePreferences, myFindChangeOptions)
	tell application "Adobe InCopy 2024"
		...
		set myScript to "tell application \"Adobe InCopy 2024\"" & return & "set properties of find glyph preferences to " & myFindPreferences & return

--CreateCharacterStyle.applescript
...
tell application "Adobe InCopy 2024"
	set myTextObjects to {text, insertion point, character, word, text style range, line, paragraph, text column}
	if (count documents) > 0 then
		...
		if (count mySelection) = 1 then
			if class of item 1 of mySelection is in myTextObjects then
				...
				if view tab of window 1 is not layout view then
					set myRestoreView to true
					set myOldView to view tab of window 1
					set view tab of window 1 to layout view
				else
					set myRestoreView to false
				end if
				...
				if myRestoreView is true then
					set view tab of window 1 to myOldView
				end if
...
on myCreateCharacterStyle(myText)
	local myClass
	set myClass to class of myText
	tell application "Adobe InCopy 2024"
